...
Application.prototype.start = function () {
  var self = this
  return self.exists()
    .then(function () { console.log('AAA'); return self.startChromeDriver() })
    .then(function () { console.log('BBB'); return self.createClient() })
    .then(function () { console.log('CCC'); return self.api.initialize() })
    .then(function () { console.log('DDD'); self.running = true })
    .then(function () { console.log('EEE'); return self })
}
...
Application.prototype.startChromeDriver = function () {
  this.chromeDriver = new ChromeDriver(this.host, this.port, this.nodePath, this.startTimeout, this.workingDirectory)
...
Application.prototype.initializeClient = function (resolve, reject) {
  ...
  var self = this
  var init = function () {
    console.log('Initialize client:', tries, this.chromeDriver);
    tries++
    self.client.init().then(() => {
        console.log('Initialization done');
        resolve();
    }, function (error) {
      console.log('Initialize client failed:', tries, error);
      if (tries >= maxTries) {
